...
//  * will branch into two recursive calls, one with `level_bool == true` and one with `level_bool
//  == false`.
//  *
//  * Once the full tuple of booleans is constructed, it is passed to the function `F`, and the
//  result
//  * is checked to see if the levels are coiterable.

// For example, say we have Levels = (A, B, C, D) with B and D unordered. The recursion would
// proceed as follows:
...
// At the end, we will have four function evaluations of `F` corresponding to the four tuples of
// booleans constructed above. If any of the function evaluations return true, then the levels are
// not coiterable.
...
// template <class F, std::size_t LevelsSize, std::size_t I, typename... PrevArgs, class Level,
// class... Levels> constexpr auto validate_boolean_args()
// {
//     static_assert(true);
// }
...
constexpr auto
constructTupleHelper(bool bool_condition)
{
    if constexpr (N == 0)
    {
        return std::tuple<>{};
    ...
    else
    {
        ...
        else
        {
            ...
            using FirstLevel =
                typename std::tuple_element<sizeof...(Ts) - N, std::tuple<Ts...>>::type;

            // Add the appropriate value to the tuple based on whether the level is ordered or
            // unordered
            if constexpr (IsOrdered<FirstLevel>::value)
            {
                return std::tuple_cat(constructTupleHelper<N - 1, Ts...>(bool_condition),
...
main()
{
    ...
    // Call check_helper with the target function 'TargetFunction' and the levels as template
    // arguments
